...
export const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    filterChallengeOld();
  }, [filteredUsersOld, filteredOldLogs]);
  ...
  useEffect(() => {
    const dataMerge = () => {
      ...
      users.map((user) => {
        ...
        // TRIVIA
        const filter_trivia = trivia.filter(
          (log) => log.idUser === user.idUser
        );
    ...
    dataMerge();
  }, [introduction, users, challenge1, challenge2, other, trivia]);
  ...
  useEffect(() => {
    ...
    dataMergeOld();
  }, [introductionOld, challenge1Old, challenge2Old, triviaOld, users]);
